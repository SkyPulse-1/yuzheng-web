    ...
    <main className="min-h-screen bg-stone-50 text-stone-900">
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link href="/libraries" className="rounded-xl bg-stone-900 px-5 py-3 text-center text-sm font-semibold text-white transition hover:bg-stone-700">+ 新建知识库</Link>
        </div>
        <section className="mt-10">
          ...
          {recentLibraries.length === 0 ? (
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{recentLibraries.map((library) => <Link key={library.id} href={`/libraries/${library.id}`} className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><p className="font-serif text-lg font-semibold">{library.name}</p><p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-stone-500">{library.description || "暂无描述"}</p><p className="mt-5 border-t border-stone-100 pt-4 text-xs text-stone-400">0 个文档</p></Link>)}</div>
          )}
        </section>
      </div>
    </main>
