...
import { splitQuestionAnswer, type QuestionCard } from "../../lib/questions";
...
export function QuestionDetailDialog({ question, onClose }: { question: QuestionCard; onClose: () => void }) {
  ...
  const conclusions = splitQuestionAnswer(question.answer || question.error || "资料中未提供足够依据。");
  useEffect(() => {
        ...
        <div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-5 lg:grid-cols-[minmax(0,1fr)_360px] lg:p-8">
          <article className="space-y-4">
            ...
            {conclusions.map((conclusion, index) => (
              <div key={`${conclusion}-${index}`} className="analysis-statement">
                <p className="text-xs font-semibold text-evidence">{String(index + 1).padStart(2, "0")}</p>
                <p className="mt-2 text-base leading-8 text-ink">{conclusion}</p>
              </div>
            ))}
          </article>
          <aside>
            <div className="flex items-center justify-between"><p className="eyebrow">原文依据</p><span className="metadata-chip">{question.evidenceCount} 条</span></div>
            <div className="mt-4 space-y-3">
              ...
              )) : <div className="source-context-placeholder">资料中未提供足够依据。</div>}
            </div>
          </aside>
        </div>
      </section>
