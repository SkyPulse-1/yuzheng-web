...
export function QuestionComposer({ selectedCount, pending, canAsk, scopeLabel, onSubmit }: {
  ...
  return (
    <form onSubmit={submit} className="question-composer">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        />
        <p className="mt-1 text-xs text-muted">{scopeLabel ?? (selectedCount ? `仅分析已选的 ${selectedCount} 份资料` : "分析当前知识库中的全部可用资料")}</p>
      </div>
