...
export function SourceContextPopover({ excerpt, sourceText }: { excerpt: SourceExcerpt; sourceText: string }) {
  ...
  const beforeSplit = Math.max(0, before.length - Math.ceil(before.length * 0.45));
  const afterSplit = Math.ceil(after.length * 0.45);
  ...
  return (
    <aside className="source-context-popover" aria-label="来源原文上下文">
      ...
      <p className="mt-3 text-sm leading-8 text-ink">
        {before ? <><span className="context-far">{before.slice(0, beforeSplit)}</span><span className="context-near">{before.slice(beforeSplit)}</span></> : null}
        <mark className="context-core">{excerpt.quote}</mark>
        {after ? <><span className="context-near">{after.slice(0, afterSplit)}</span><span className="context-far">{after.slice(afterSplit)}</span></> : null}
      </p>
